...
int network(int *ani, int *vec, int l, int k) {
  ...
  for (i=k;i<l+k;i++) {
    for(j=1;j<k+1;j++) {
      *(vec+(i-k)*2*k+j-1) = i-k+j-2; //Los k vecinos de la izquierda.
    }
    for (j=k+1;j<2*k+1;j++) {
      *(vec+(i-k)*2*k+j-1) = i-k+j-1; //Los k vecinos de la derecha.
    }
  }
  ...
  for (i=0;i<k;i++) { //Para el primer paciente y el último, respectivamente.
    *(vec+i) = l+i-1; //Toma los valores del final.
    *(vec+(l-1)*2*k+2+i) = k+i-1; //Toma los valores del principio.
  }
  *(vec+2*k) = l; //Toma el valor correcto del primer vecino del segundo paciente.
