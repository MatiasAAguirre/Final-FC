...
int main(int argc,char*argv[]) {
  ...
  int i, l, k, Te, r, inf, cant_inf, t_inf, n_ref;
  float p, rho, t, r_fm, r_fm2, gamma_i, gamma_c;

  sscanf(argv[1],"%d",& l);
  sscanf(argv[2],"%d",& k);
  sscanf(argv[3],"%f",& p);
  sscanf(argv[4],"%f",& rho);
  sscanf(argv[5],"%f",& gamma_i);
  sscanf(argv[6],"%f",& gamma_c);
    ...
    while (cant_inf > 0) {
      ...
      if (t-t_inf>Te) {
        *(pac+inf) = 1; //Cambia a estado refractario.
        ...
        r_fm = ((float)rand())/((float) RAND_MAX);
        r_fm2 = ((float)rand())/((float) RAND_MAX);
